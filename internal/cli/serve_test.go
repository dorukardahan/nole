...
func TestHTTPServerTimeoutsLocked(t *testing.T) {
	// Regression-lock the deliberate timeouts — especially the 300s WriteTimeout
	// sized for the worst-case provider fallback chain (Codex PR #25). A future
	// "tidy-up" that shrinks WriteTimeout to e.g. 60s would guillotine a
	// legitimate handler mid-fallback; this test makes that change conscious.
	readHeader, read, write, idle := httpServerTimeouts()
	if readHeader != 5*time.Second {
		t.Errorf("ReadHeaderTimeout = %v, want 5s", readHeader)
	}
	if read != 30*time.Second {
		t.Errorf("ReadTimeout = %v, want 30s", read)
	}
	if write != 300*time.Second {
		t.Errorf("WriteTimeout = %v, want 300s (provider-fallback headroom; do not shrink)", write)
	}
	if idle != 120*time.Second {
		t.Errorf("IdleTimeout = %v, want 120s", idle)
	}
}
