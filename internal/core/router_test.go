...
func TestRouterGeneralPrefersBraveThenTavilyThenDDGS(t *testing.T) {
...
func TestRouterFallsBackWhenQuotaExhausted(t *testing.T) {
	...
	if provider.Name() != "tavily" {
		t.Fatalf("expected tavily fallback, got %q", provider.Name())
	}
}
...
func TestRouterReturnsNoFreeQuota(t *testing.T) {
	...
	ledger := NewMemoryQuotaLedger()
	ledger.Set(QuotaEntry{Provider: "brave", FreeRemaining: 0})
	router := NewRouter(registry, ledger, RouteMatrix{TaskGeneral: []string{"brave"}})
...
func TestRouterExtractPrefersJinaThenFirecrawl(t *testing.T) {
	registry := NewRegistry()
	...
	if provider.Name() != "jina" {
		t.Fatalf("expected jina, got %q", provider.Name())
	}
}
