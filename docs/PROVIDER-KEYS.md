...
| Firecrawl | `account_usage_endpoint` when keyed; `not_applicable` in keyless mode | Keyed mode calls `GET /team/credit-usage`; keyless mode has no account balance to query and is governed upstream by per-IP daily request and credit limits. |
...
- **`FIRECRAWL_API_KEY` present:** The same Firecrawl capability switches from per-IP keyless limits to account-backed quota/higher upstream limits; it does not unlock a previously missing tool. Firecrawl currently documents 1,000 credits as the free API-key signup allowance, not as the keyless allowance.
...
`provider_status` returns a `setup_suggestions` array for missing keys that unlock a genuinely key-gated provider, including what configuring it would unlock, where to sign up, and an `impact` rating (`high` / `medium` / `low`). It deliberately does not suggest `FIRECRAWL_API_KEY`, because Firecrawl already works keylessly; provider status still explains that an existing key enables account-backed quota/higher limits. The first `search` response of an MCP session may carry a compact `setup_tip`; it remains an optional speed/fidelity upgrade hint for the remaining keyed providers, never a Firecrawl credential prompt or disabled-feature warning. Subsequent searches in the same session omit it to avoid nagging.
