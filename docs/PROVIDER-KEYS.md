...
**OpenClaw-only host mode:** `nole setup --openclaw` does not use Nólë's direct Firecrawl credential path. Its dedicated wrapper invokes supported Firecrawl operations through OpenClaw's authenticated `tools.invoke`; no Firecrawl key or Gateway token is copied into Nólë/OpenClaw MCP config. If the installed plugin advertises `firecrawl-free`, Nólë delegates search and extract. Current stable plugins provide OpenClaw `web_fetch` with keyless Firecrawl fallback but key-gate search, so Nólë delegates host fetch and routes search through its existing fallbacks. This mode is scoped to `nole-mcp-openclaw`. All other clients retain the direct API behavior described below.
