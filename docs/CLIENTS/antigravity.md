...
Remote MCP entries may use `serverUrl`, but Nólë's writer does not create a remote entry. It preserves existing remote/sibling servers while upserting the local `nole` server.
