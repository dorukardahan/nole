...
| Hermes Agent | verified (Hermes Agent MCP profile path) | `nole setup --hermes --local-extract` or equivalent wrapper-direct MCP config | Verified on Hermes Agent v0.19.0 (v2026.7.20) through the real MCP client path with all six native Nólë tools. The current check reused existing configuration and performed no binary replacement, MCP reload, or service restart. See `docs/CLIENTS/LIVE-VERIFICATION.md`. |
...
| Antigravity CLI | repo-tested | `nole setup --antigravity` (writes `~/.gemini/config/mcp_config.json`) | Antigravity CLI (`agy`) is the current consumer successor and reads the global MCP config at `~/.gemini/config/mcp_config.json`; Nólë's writer upserts a local stdio `mcpServers.nole` entry while preserving sibling servers and remote `serverUrl` entries. Stays `repo-tested` until authenticated `/mcp` or `agy -p` tool visibility is observed. See `docs/CLIENTS/antigravity.md`. |
| Gemini CLI | repo-tested | `nole setup --gemini` (writes `~/.gemini/settings.json`) or `gemini mcp add --scope user nole /absolute/path/to/nole mcp` | Preserved for Gemini CLI Standard/Enterprise/Cloud/paid API-key users. 2026-06-04 live run (Gemini CLI 0.40.1): the writer's path + `mcpServers` schema match the client's own `gemini mcp add` output. Stays `repo-tested` because in-client tool visibility was not observable. See `docs/CLIENTS/gemini.md` + `LIVE-VERIFICATION.md`. |
| Grok CLI (superagent) | repo-tested | `nole setup --grok` (writes `~/.grok/user-settings.json`) | Targets `superagent-ai/grok-cli` (JSON `mcp.servers` array, upsert-by-`id`, repo-tested). It was not installed on the 2026-06-04 host (the host's `grok` was xAI's Grok Build TUI — see the next row). For xAI's Grok Build TUI use `--grok-build`, not `--grok`. See `docs/CLIENTS/grok.md`. |
...
7. Tool visibility for `search`, `extract`, `search_and_extract`, `research`, `provider_status`, and `budget_status` (`extract` and `search_and_extract` are advertised out of the box via the keyless httpfetch backstop; a Tavily/Firecrawl key or local Scrapling upgrades extract fidelity).
