...
| Gemini CLI | repo-tested | `nole setup --gemini` (writes `~/.gemini/settings.json`) or `gemini mcp add --scope user nole /absolute/path/to/nole mcp` | Setup writer + config merge covered by repo tests; the real Gemini CLI was not launched in this environment, so tool visibility is unobserved. `mcpServers` object keyed by name, structure verified from primary source (`google-gemini/gemini-cli`). See `docs/CLIENTS/gemini.md`. |
| Grok CLI | repo-tested | `nole setup --grok` (writes `~/.grok/user-settings.json`) | Setup writer + array upsert-by-`id` covered by repo tests; the real Grok CLI was not launched in this environment, so tool visibility is unobserved. `mcp.servers` array keyed by `id`, structure verified from primary source (`superagent-ai/grok-cli`); no first-class `mcp add` CLI (use the writer, the `/mcps` TUI, or direct JSON). See `docs/CLIENTS/grok.md`. |
