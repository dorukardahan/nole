...
- The writer manages the `[mcp_servers.nole]` launch keys (`command`/`args`/`enabled`): re-running `nole setup --grok-build` rewrites them, and **other** MCP servers + root keys are preserved. A user-set `enabled = false` IS preserved across re-runs. If you have hand-customized the nole entry with content the writer does not manage — a `[mcp_servers.nole.*]` sub-table (e.g. `[mcp_servers.nole.env]` env overrides) or an extra direct key — the writer **refuses to overwrite it** (it leaves the file untouched and tells you), rather than silently dropping your settings; update `command`/`args` there by hand, or remove the customization and re-run. For provider keys, prefer `~/.config/nole/.env` + the wrapper (below) over an `env` sub-table.
