# Nólë MVP Implementation Plan

> **Implementation note:** Follow TDD — write failing tests first, then implement to pass.

**Goal:** Build the first MVP of `nole`: a Go single-binary agent search/retrieval router with MCP stdio, CLI, task-based provider routing, and $0/free-tier fail-closed policy.
...
- **Name:** repository/package/binary name is `nole` (lowercase ASCII for GitHub/tooling compatibility); visible product name in docs is **Nólë**.
- **Name origin:** Nólë is Quenya (Tolkien's High Elvish) for deep knowledge/research: knowledge gained through mental seeking, careful study, and investigation.
- **Language:** Go.
...
nole/
  ...
  main.go
  README.md
  docs/plans/2026-05-12-nole-mvp.md
  internal/
    cli/
      root.go
      search.go
...
Use Cobra. Root command should set `Use: "nole"`, `Short`, `SilenceUsage: true`, and `SilenceErrors: true`.
...
Commands:
- `nole search "query" --task general --json`
...
- `server.NewMCPServer("nole", version.Version, server.WithToolCapabilities(false))`
- `mcp.NewTool("search", ...)`
- `server.ServeStdio(s)`
...
go build -o ./bin/nole .
./bin/nole doctor
./bin/nole providers --json
./bin/nole search "model context protocol go sdk" --task general --json
```
