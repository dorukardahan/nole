...
Build toolchain: `go 1.25.13`. CI also invokes `golang.org/x/vuln/cmd/govulncheck` (pinned in `ci.yml`/`release.yml`), the `gh` CLI, and `python3` (for `secret-scan.sh` and the orphaned `cmd/bench/main.py`).
