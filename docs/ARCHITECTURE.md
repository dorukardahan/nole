...
| `brave.Provider.Search` | method | `internal/providers/brave/brave.go:81` | GET w/ `X-Subscription-Token`; clamps count to the selected endpoint cap; uses Web Search for general/non-recency tasks and News Search for `news`/`factcheck`; maps provider `web.results` or News Search top-level `results`. |
...
7. **Provider call.** `provider.Search(ctx, req)` (e.g. `brave.go:81`), routed through `providerhttp.DoWithRetry` (`retry.go:40`) for network providers; latency via `time.Since`. Error → `provider_error` trace, continue; empty results → `empty_results` trace, continue (free tier NOT debited).
