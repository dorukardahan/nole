...
If `go` is missing, install Go 1.25+ (matches the `go 1.25.10` directive in `go.mod`) using the user's normal package manager or a user-local toolchain. Do not commit downloaded toolchains or build artifacts.
